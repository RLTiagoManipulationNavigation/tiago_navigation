...
import numpy as np
from ddpg.model import (ActorNet , CriticNet)
from ddpg.replay_buffer import Replay_Buffer
from ddpg.attention_module import Spatial_Attention
import rospy
from ddpg.OUNoise import OUNoise
from ddpg.utils import clone_model , target_weight_update
...
class DDPG(nn.Module):
    def __init__(self, env):
        ...
        self.ddpg_input_dim = rospy.get_param("/DDPG/spatial_att_ourdim")
        self.spatial_input_size = rospy.get_param("/DDPG/input_spatial_size")
        ...
        self.replay_buffer = Replay_Buffer(rospy.get_param('/DDPG/buffer_size'))

        #initialize the optimizer
        self.actor_optimizer = torch.optim.Adam(self.actor_network.parameters(), lr=rospy.get_param("/ADAM_param/actor"))
        self.critic_optimizer = torch.optim.Adam(self.critic_network.parameters() , lr=rospy.get_param("/ADAM_param/critic"))
        self.attention_optimizer = torch.optim.Adam(self.spatial_attention.parameters() , rospy.get_param("/ADAM_param/spatial_attention"))
